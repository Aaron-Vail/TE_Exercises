...
SELECT name, governmentform FROM country WHERE continent = 'Europe';
...
SELECT name, region FROM country WHERE region != 'Caribbean' AND continent LIKE '%America%';
...
SELECT ROUND((gnp / population)* 1000000, 2) FROM country WHERE continent = 'Europe';
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT (2017 - indepyear) AS years_since_independance FROM country WHERE indepyear IS not null;